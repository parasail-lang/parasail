...
--  This algorithm is a sequential quicksort

with Ada.Text_IO; use Ada.Text_IO;

procedure Gen_Qsort (First, Last : Index_Type'Base) is
   ...
   procedure Sort_Slice (Orig_First : Index_Type; Orig_Last : Nat_Index_Type)
   is
      --  Sort given slice of Arr and spawn sorts of bigger half
      First : Index_Type := Orig_First;
      Last : Nat_Index_Type := Orig_Last;
   begin
      if Debug then
         ...
         if Last = Index_Type'Succ (First) then
            ...
            begin
                  ...
                  begin
                     begin
                        ...
                        for I in Right_First .. Left_Last loop
                           if not Before (I, Mid_Index) then
                              --  Found an item OK for right partitition
                              Swap_Left := I;
                              exit when Before (Mid_Index, I);
                     ...
                     begin
                        ...
                        for J in reverse Right_First .. Left_Last loop
                           if not Before (Mid_Index, J) then
                              --  Found an item OK for left partitition
                              Swap_Right := J;
                              exit when Before (J, Mid_Index);
                     ...
                     --  Track where the pivot has moved
                     if Mid_Index = Swap_Left then
                        Mid_Index := Swap_Right;
                     elsif Mid_Index = Swap_Right then
                        Mid_Index := Swap_Left;
                     end if;
               ...
               if Debug then
                  ...
                  loop
                     if Before (Mid_Index, K)
                       or else Before (K, Mid_Index)
                     then
                        Put_Line ("*** Arr (" & K'Image & ") /= Pivot");
                        exit;
                     end if;
                  end loop;
               end if;
               if Checks and then not
                 ((for all K in
                     Index_Type'Succ (Left_Last) ..
                       Index_Type'Pred (Right_First) =>
                         not Before (Mid_Index, K)
                           and then not Before (K, Mid_Index)))
               then
...
begin  --  Seq_Qsort
   ...
   --  Start the sort, giving the full bounds of the array.
   if First < Last then
      Sort_Slice (First, Last);
   end if;
