...
with System;
with czmq_library_h;
package ZMQ.Zsocks is

   type Zsock is tagged private;
   ...
   --  CZMQ_EXPORT zsock_t *
   --  zsock_new (int type);
   function Zsock_New (Kind : Integer) return Zsock;
   ...
   function Zsock_New_Pair (Endpoint : String) return Zsock;
   ...
   procedure Zsock_Destroy (Self_P : in out Zsock);
   ...
   function Zsock_Bind (Self : Zsock; Endpoint : String) return Integer;
   ...
   function Zsock_Connect (Self : Zsock; Endpoint : String) return Integer;
   ...
   --  For types 1, 2, 4 and 8 caller must allocate the memory itself before
   --  calling zsock_brecv.
   ...
   --  Set socket to use unbounded pipes (HWM=0); use this when you are
   ...
   --  discard it. This is useful when reading partial messages, to get
   ...
   --  Probe the supplied reference. If looks like a zsock_t instance, return
   ...
   --     (void *self, int xpub_manual_last_value);
   ...
   --     (void *self, int gssapi_principal_nametype);
   ...
   --  Available from libzmq 4.3.0.
   --  CZMQ_EXPORT void
   --  zsock_set_gssapi_service_principal_nametype
   --     (void *self, int gssapi_service_principal_nametype);
   ...
   --     (void *self, const char *gssapi_service_principal);
   ...
   --     (void *self, int delay_attach_on_connect);
   ...
   function Zsock_Addr (Self : Zsock) return System.Address;
   --  Return an address that can be used for socket operations
   --  that expect a system.address.

   type Zsock_T_Access is access all czmq_library_h.zsock_t;

   function GetImplAcc (Self : in out Zsock) return access Zsock_T_Access;

private

   type Zsock is tagged record
      Raw : aliased Zsock_T_Access;
   end record;

   function Zsock_Addr (Self : Zsock) return System.Address
     is (Self.Raw.all'Address);
