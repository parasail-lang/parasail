...
--          Put_Line("Filename " & To_String(Filename));
    ...
    begin
        c_lexer_dfa.yy_init := True;
        Source_Contents := Str;
    end Set_Input_Buffer;

    procedure YY_INPUT(buf: out c_lexer_dfa.unbounded_character_array;
...
--          Std_Output.Put(c);
        ...
        if i + offset > Source_Contents'Last then
          ...
          c_lexer_io.yy_eof_has_been_seen := True;
          read_from := 0;
        else
        ...
        c_lexer_io.YY_INPUT_default(buf, result, max_size);
        return;
    end if;
