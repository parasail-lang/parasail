...
Line_Terminator : constant Wide_Character := 
  Wide_Character'Val(Character'Pos(ASCII.LF));
Vertical_Tab    : constant Wide_Character := 
  Wide_Character'Val(Character'Pos(ASCII.VT));
Page_Terminator : constant Wide_Character := 
  Wide_Character'Val(Character'Pos(ASCII.FF));
Nul_Character : constant Wide_Character := Wide_Character'Val(0);
Horizontal_Tab : constant Wide_Character := 
...
type wint_t is new Interfaces.C.int;
pragma Import(Intrinsic, wint_t, "#include <wchar.h>", "wint_t");
  -- This type is sometimes unsigned in C, so we use import
  -- intrinsic to get it

c_WEOF : constant wint_t := -1;  -- Returned from c_fgetwc on EOF

function c_fgetwc ( Fileid : Stdio.File_Ptr) return wint_t;
pragma Import (c, c_fgetwc, "fgetwc");

function c_ungetwc (
   Ch : wint_t;   
   Fileid : Stdio.File_Ptr) return wint_t;
pragma Import (Intrinsic, c_ungetwc, "ungetwc");
  -- Can't seem to get an exact match with MingW version
  -- so using Intrinsic convention.

function c_fputwc ( Ch : wchar_t;
  Fileid : Stdio.File_Ptr ) return wint_t;
pragma Import (c, c_fputwc, "fputwc");

function c_fputws ( S : Interfaces.C.wchar_array;
  Fileid : Stdio.File_Ptr ) return Integer;
pragma Import (c, c_fputws, "fputws");
...
procedure Skip_Line (File : in File_Type;
    ...
    Char : Wide_Character := Nul_Character;
    ch : wint_t;
    Result : wint_t;
begin
    Check_Mode_In(File);
    ...
    for i in 1 .. Spacing loop
		...
		raise END_ERROR;
            ...
            Char := Wide_Character'Val(ch);
	    File.Current_Column := File.Current_Column + 1;
            exit when Char = Line_Terminator or else Char = Page_Terminator;
	...
	if Char = Page_Terminator then
		...
		-- so put back the Wide_Character.
		Result := c_ungetwc(ch, File.CFile);
	    end if;
	end if;
...
begin
    ...
    Char := Wide_Character'Val(ch);
    -- Push back first Wide_Character
    Result := c_ungetwc(ch, File.CFile);
    return Char  = Line_Terminator;
end End_Of_Line;
...
begin
	...
	Skipped_At_Least_One_Character := True;
        Char := Wide_Character'Val(ch);
        File.Current_Column := File.Current_Column + 1;
        if Char = Line_Terminator then
            Next_Line(File);
        end if;
        exit when Char = Page_Terminator ;
...
begin
    ...
    if Item = Line_Terminator then
	chnext := c_fgetwc(File.CFile);
	if chnext = c_WEOF then
	    -- End of file
	    Result := c_ungetwc(ch, File.CFile);
	    return True;
	end if;
	-- Push back second character
	Result := c_ungetwc(chnext, File.CFile);

	-- Now decide whether at end of page
        Item := Wide_Character'Val(chnext);
    end if;
    -- Push back first character
    Result := c_ungetwc(ch, File.CFile);
	
    return Item = Page_Terminator;
...
begin
    ...
    if Item = Page_Terminator then
	chlast := c_fgetwc(File.CFile);
	if chlast = c_WEOF then
	    -- End of file
	...
	Result := c_ungetwc(chlast, File.CFile);
    end if;
			...
			raise END_ERROR;
	            ...
	            File.Current_Column := File.Current_Column + 1;
                    exit when (File.Current_Column - 1)  = To  and 
                     Item /= Line_Terminator and Item /= Page_Terminator ;

		    if Item = Line_Terminator then
                        -- EOL detected
                        Next_Line(File);
                    elsif Item = Page_Terminator then
                        -- EOP detected 
                        Next_Page(File);
		    else
                        -- Non terminator - 
...
begin
    ...
    -- reads the next Wide_Character from the file 
    Check_Mode_In(File);
...
begin
    ...
    case Item is
        ...
        when others =>
            ...
            ch := c_fputwc(wchar_t(Item), File.CFile);
            File.Current_Column := File.Current_Column + 1;
    end case;
end Put;
...
begin
    ...
    Item := Wide_Character'Val(ch);
    if Item = Line_Terminator or Item = Page_Terminator then
        Item := Nul_Character;
        End_Of_Line := true;
    else
        End_Of_Line := false;
...
procedure Get_Line(Item : out Wide_String;
                   Last : out Natural) is
begin
    Check_Current_Input;
...
procedure RTS_LEX_FILE(FILE : FILE_TYPE;
            ...
            WIDTH: FIELD;
            RESULT: out Wide_String; -- resulting lexical unit withOUT
...
--  dbg : constant boolean := false;
    ...
    Last_Char_To_Push_Back : wint_t := c_WEOF;
    ...
    begin
		...
		raise END_ERROR;
            ...
            CHAR := Wide_Character'Val(ch);
            -- what about TABS
	    if CHAR /= ' ' and then CHAR /= Horizontal_Tab then
		-- Not a blank

		exit when CHAR /= Line_Terminator and then 
		  CHAR /= Page_Terminator;
    ...
    --|    Last_Char_To_Push_Back if /= c_WEOF on return should be
    --|     "pushed back" (i.e. passed to ungetwc).
    ...
    begin
	    ...
	    Last_Char_To_Push_Back := c_WEOF;  -- Nothing to push back
	    return ' ';
	end if;

	Last_Char_To_Push_Back := c_fgetwc(File.CFile);
	if Last_Char_To_Push_Back = c_WEOF then
	    -- EOF detected
	...
	Item := Wide_Character'Val(Last_Char_To_Push_Back);
	if Item = Line_Terminator or else Item = Page_Terminator then
	    -- Line/page terminator, return a space.
	    Last_Char_To_Push_Back := c_ungetwc(Last_Char_To_Push_Back,
	      File.CFile);
	    Last_Char_To_Push_Back := c_WEOF;
	    return ' ';
	end if;
...
begin
	...
	if Last_Char_To_Push_Back /= c_WEOF then
	    ...
	    ch := c_ungetwc(Last_Char_To_Push_Back, File.CFile);
...
--      end if;     --dbg_off
        ...
        if width > Num_Chars_Read then
	        ...
	        ch := c_fgetwc(File.CFile);
	        exit when ch = c_WEOF; -- EOF detected
                Char := Wide_Character'Val(ch);
                if Char = Line_Terminator or else Char = Page_Terminator then
...
int_to_char : constant Wide_String (1 .. 16) := "0123456789ABCDEF";
...
-------------------------------------------------------------
    ...
    is
        buffer : Wide_String (1 .. 256); -- big enough for non-blank part of
                                    -- any integer
        last : natural;
    begin
        RTS_LEX_FILE (FILE,
        ...
        begin
            ...
            ITEM := TGT.Longest_Integer(
                      System.RTS.Integer_Value_Pkg.RTS_Integer_Value(
                        ACH.To_String(BUFFER(BUFFER'first .. last))));
        exception
...
-------------------------------------------------------------
    ...
    begin
        System.RTS.Lexer.RTS_LEX_STRING(ACH.To_String(FROM),
                       System.RTS.Integer_State_Tables.Transitions,
                       System.RTS.Integer_State_Tables.Mapping,
                       MY_Last);
        LAST := My_Last;
        begin
            ...
            ITEM := TGT.Longest_Integer(
                      System.RTS.Integer_Value_Pkg.RTS_Integer_Value(
                        ACH.To_String(FROM(My_first .. My_Last))));
        exception
            when CONSTRAINT_ERROR =>
                raise DATA_ERROR;
        end;
...
-------------------------------------------------------------
   ...
   put_integer_string_internal_error : exception;
     ...
     -- implying integer_image didn't pad with blanks
       ...
       item_image : constant Wide_String :=
...
--|   PUT(file) and PUT(string) have already checked that base is correct.
    ...
    buf : Wide_String (index_subtype);

    X : TGT.Longest_Integer := ITEM;

    use type TGT.Longest_Integer;

    procedure ADD (C : Wide_Character)
    --+-----------------------------------------------------
    --| Overview
...
-------------------------------------------------------------
    ...
    is
        buffer : Wide_String (1 .. 512); -- big enough for non-blank part of
                                    -- any real
        last : natural;
    begin
        RTS_LEX_FILE (FILE,
...
-------------------------------------------------------------
    ...
    begin
        ...
        PUT_WORD (file => FILE,
                  word => ACH.To_Wide_String(
		    FIO.FLOAT_IMAGE (item, fore, aft, exp)),
                  width => 0 ); -- no padding needed
    ...
    begin
        ...
        begin
            ITEM := FIO.FLOAT_VALUE (
	      ACH.To_String(FROM(FROM'first .. My_Last)));
        exception
            when CONSTRAINT_ERROR =>
                raise DATA_ERROR;
        end;
...
function ENUM_IMAGE (  --| performs requested conversion lower case and
                   ...
                   IMAGE : Wide_String;
                   WIDTH : FIELD;
                   SET   : TYPE_SET) return Wide_String
--+-----------------------------------------------------
--| Effect
...
is
    ...
    Is_Char_Lit : constant Boolean := (IMAGE(IMAGE'first) = ''');
begin
    if length >= width then
	-- No need to pad with blanks, if don't have to
	-- convert to lower case either, then just return
	if SET = UPPER_CASE or else Is_Char_Lit
	    -- No conversion on char lits
	  then
	    return IMAGE;
	end if;
    end if;
    if SET = LOWER_CASE and then not Is_Char_Lit then
        -- Have to convert to lower case
	return ACH.To_Wide_String(ACH.To_Lower(ACH.To_String(IMAGE))) &
	  (length+1 .. width => ' ');
    else
	return IMAGE & (length+1 .. width => ' ');
    end if;
end ENUM_IMAGE;
...
--|   Generate the ENUM value via ENUM'Wide_VALUE,
--| converting any constraint error into DATA_ERROR.

is
    buffer : Wide_String (ENUM_Width); -- big enough for non-blank part of
    ...
    begin
        while first in BUFFER'first..last loop
            exit when (BUFFER(first) /= ' ') and then
              (BUFFER(first) /= Horizontal_Tab);
            first := first + 1;
        end loop;
        ITEM := ENUM'wide_value (BUFFER(first .. last));
    exception
        when CONSTRAINT_ERROR =>
            raise DATA_ERROR;
    end;
...
begin
    ...
    Check_Mode_Out_Or_Append(File);
    PUT_WORD (file => FILE,
              word => ENUM_IMAGE (Enum'Wide_IMAGE(Item), 1, set),
              width => WIDTH);
end PUT;  -- file
    ...
    begin
        while My_First in FROM'first..My_Last loop
            exit when (FROM(My_First) /= ' ') and then
              (FROM(My_First) /= Horizontal_Tab);
            My_First := My_First + 1;
        end loop;
        ITEM := ENUM'wide_value (FROM(My_First .. My_Last));
    exception
        when CONSTRAINT_ERROR =>
            raise DATA_ERROR;
    end;
...
procedure PUT( TO : out Wide_String;
               ...
               SET : in TYPE_SET := default_setting ) is
   item_image : constant Wide_String :=
           enum_image(Enum'Wide_IMAGE(Item), To'LENGTH, Set);
...
begin
    if File.Line_Length = Unbounded then
        ...
        Result := c_fputws(Interfaces.C.To_C(Item), File.CFile);
...
begin
    for I in 1 .. Spacing loop
        Result_wc := c_fputwc(wchar_t(Line_Terminator), File.CFile);
        Next_Line(File);
        if File.Page_Length /= Unbounded and then
...
begin
    if File.Current_Column /= 1 or else File.Current_Line = 1 then
        ...
        Result := c_fputwc(wchar_t(Line_Terminator), File.CFile );
    end if;
    Result :=  c_fputwc(wchar_t(Page_Terminator), File.CFile );
...
begin
	...
	loop
	    ...
	    Char := Wide_Character'Val(ch);
            if Char = Line_Terminator then
                Next_Line(File);
            elsif Char = Page_Terminator then
                Next_Page(File);
            else
		exit;
	    end if;
...
begin
    ...
    for i in Item'FIRST .. Item'LAST loop
        ...
        Item(i) := Wide_Character'Val(ch);
        Skipped_At_Least_One_Character := True;
        if Item(i) = Line_Terminator then
            -- consume EOL
            Last := Last - 1;
            exit;
        end if;
