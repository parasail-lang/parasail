...
package body Ada.Containers.Hashed_Maps is
   ...
   function Chunk_Count (Object : Iterator) return Chunk_Index is
   begin
      if Object.Num_Chunks <= 0 then
         raise Program_Error;
      else
         return Object.Num_Chunks;
      end if;
   end Chunk_Count;
   ...
   function First (Object : Iterator;
      ...
      Num_Chunks : constant Chunk_Index'Base := Object.Num_Chunks;
      Buckets    : constant Buckets_Access := Object.Container.HT.Buckets;
   begin
      ...
      --  Iterate through the buckets that make up this chunk
      declare
         This_Chunk_Start : constant Hash_Type :=
           Buckets'Length * Hash_Type (Chunk - 1) / Hash_Type (Num_Chunks);
         This_Chunk_End   : constant Hash_Type :=
           Buckets'Length * Hash_Type (Chunk) / Hash_Type (Num_Chunks) - 1;
      begin
         for Pos in This_Chunk_Start .. This_Chunk_End loop
            declare
               Node : constant Node_Access := Buckets (Pos);
            begin
               --  Found a non-empty bucket
               if Node /= null then
                  return (Object.Container, Node, Pos);
               end if;
            end;
         end loop;
   ...
   function Is_Split (Object : Iterator) return Boolean is
   begin
      return Object.Num_Chunks > 0;
   end Is_Split;
   ...
   function Iterate
     (Container : Map) return Map_Iterator_Interfaces.Parallel_Iterator'Class
   is
   begin
   ...
   function Next
     (Object   : Iterator;
      ...
      Orig_Pos : constant Hash_Type := Position.Position;
   ...
   begin
      ...
      if Node = null then
         return No_Element;
      elsif Pos > Orig_Pos then
         --  A new bucket -- see if still in this chunk
         declare
            Next_Chunk_Start : constant Hash_Type :=
              Position.Container.HT.Buckets'Length * Hash_Type (Chunk) /
                Hash_Type (Num_Chunks);
         begin
            if Pos >= Next_Chunk_Start then
   ...
   procedure Split_Into_Chunks
     (Object     : in out Iterator;
      ...
      Buckets : constant Buckets_Access := Object.Container.HT.Buckets;
      Length  : constant Natural := Natural (Object.Container.HT.Length);
   begin
      if Is_Split (Object) then
         raise Program_Error;
      elsif Buckets = null or else Length = 0 then
         --  Map is empty, so no need for more than one chunk
         Object.Num_Chunks := 1;
      else
         --  At most one chunk per bucket, and at most one chunk per element.
         Object.Num_Chunks :=
           Chunk_Index'Min
             (Chunk_Index'Min (Buckets'Length, Chunk_Index (Length)),
              Max_Chunks);
      end if;
