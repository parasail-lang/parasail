...
package body System_Put_Images is
   use Ada2X_Strings.Text_Buffers;

   procedure Put_7bit (S : in out Sink'Class; Item : String) is
   begin
      Put_UTF_8 (S, Item);
   end Put_7bit;

   procedure Put_7bit (S : in out Sink'Class; Item : Character) is
   begin
      Put_UTF_8 (S, (1 => Item));
   end Put_7bit;
   ...
   procedure Put_Wide_Character (S : in out Sink'Class;
                                 Item : Wide_Character) is
   begin
      Wide_Put (S, Wide_String'(1 => Item));
   end Put_Wide_Character;

   procedure Put_Wide_Wide_Character (S : in out Sink'Class;
                                      Item : Wide_Wide_Character) is
   begin
      Wide_Wide_Put (S, Wide_Wide_String'(1 => Item));
